    ...
    const pml::vec3& bottomLeft) 
{
    ...
    // each vertex is 7 floats, 3 for position, 4 for colour
    ...
    // add vertices
    std::unique_ptr<VertexContainer> container = std::make_unique<VertexContainer>(vertexDescription);
    container->addRawData(topLeft);     
    container->addRawData(colour);
    container->addRawData(topRight);
    container->addRawData(colour);
    container->addRawData(bottomRight);
    container->addRawData(colour);
    container->addRawData(bottomLeft);
    container->addRawData(colour);
    ...
    const pml::vec3& c) 
{
    ...
    // add vertices
    std::unique_ptr<VertexContainer> container = std::make_unique<VertexContainer>(vertexDescription);
    container->addRawData(a);     
    container->addRawData(colour);
    container->addRawData(b);
    container->addRawData(colour);
    container->addRawData(c);
    container->addRawData(colour);
    ...
    const pml::FLOAT dimension) 
{
    ...
    // add vertices
    std::unique_ptr<VertexContainer> container = std::make_unique<VertexContainer>(vertexDescription);
    container->addRawData(pml::vec3(0.0f, 0.0f, 0.0f));     
    container->addRawData(colour);
    container->addRawData(pml::vec3(dimension, 0.0f, 0.0f));
    container->addRawData(colour);
    container->addRawData(pml::vec3(dimension, 0.0f, dimension));
    container->addRawData(colour);
    container->addRawData(pml::vec3(0.0f, 0.0f, dimension));
    container->addRawData(colour);
    container->addRawData(pml::vec3(0.0f, dimension, 0.0f));
    container->addRawData(colour);
    container->addRawData(pml::vec3(dimension, dimension, 0.0f));
    container->addRawData(colour);
    container->addRawData(pml::vec3(dimension, dimension, dimension));
    container->addRawData(colour);
    container->addRawData(pml::vec3(0.0f, dimension, dimension));
    container->addRawData(colour);
    ...
    const std::vector<std::uint32_t>& indices)
{
    ...
    // generate hash
    std::size_t meshId = prism::hash(vertexContainer->getVertexStride(), vertexContainer->getDataSize(), vertexContainer->getNumVertices(), indices.size());
